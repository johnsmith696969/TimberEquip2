...
- staged lifecycle validation now has a repeatable smoke script and evidence artifact for create -> reject -> resubmit -> approve -> payment_confirmed -> mark_sold -> relist -> archive, including the admin lifecycle audit endpoint
- staged billing validation now has a repeatable checkout-start script and evidence artifact for `Owner-Operator`, `Dealer`, and `Pro Dealer` plan starts
- the staged seller-plan return path is now materially healthier: `/subscription-success` no longer bounces a completed checkout back to login, and a real March 29, 2026 staging `Owner-Operator` payment was verified through to an active seller account state
- operator-facing governance surfaces are now materially better: admin billing exposes account governance plus legal acceptance panels, admin machines exposes review filters and review-summary counts, and dealer feeds exposes operator summaries for last run, failure reason, and ingest/profile state
