...
- `timberequip-staging` now has Hosting, Monitoring, a default Firestore database in `nam5`, Blaze billing, and the Cloud Functions v2 stack enabled
- staging now has deployed `apiProxy` and `publicPages`
- staging browser builds now use staging Firebase config instead of falling back to production
- the authenticated `/api/account/bootstrap` slice has now been promoted through staging and production
- the consolidated admin billing/content bootstrap slice has now also been promoted through staging and production
- the consolidated admin operations bootstrap slice for users, accounts, inquiries, and calls has now also been promoted through staging and production
- the dealer-feed bootstrap slice has now also been promoted through staging and production, and DealerOS now consumes the same safer dealer-feed bootstrap contract
- route smoke checks passed on March 28, 2026 for both `https://timberequip-staging.web.app` and `https://www.timberequip.com`
- remaining operational gap: GitHub Environment promotion and reviewer flow still needs to become the default release path
