...
interface AccountMfaSettingsCardProps {
  user: UserProfile | null | undefined;
  onProfilePatch?: (patch: Partial<UserProfile>) => void;
  className?: string;
}

export function AccountMfaSettingsCard({ user, onProfilePatch, className = '' }: AccountMfaSettingsCardProps) {
  const recaptchaId = `account-mfa-recaptcha-${useId().replace(/:/g, '')}`;
  ...
  useEffect(() => {
    ...
    setMfaPhoneNumber(user?.mfaPhoneNumber || user?.phoneNumber || '');
    setMfaDisplayName(user?.mfaDisplayName || user?.displayName || '');
    setMfaVerificationId('');
    setMfaVerificationCode('');
    setMfaError('');
    setMfaNotice('');
  }, [user?.displayName, user?.mfaDisplayName, user?.mfaPhoneNumber, user?.phoneNumber, user?.uid]);
  ...
  const syncMfaProfileState = async (factors: SmsMfaFactorSummary[]) => {
    ...
    await withAsyncTimeout(
      userService.updateProfile(user.uid, patch),
      15000,
  ...
  const getMfaRecaptcha = async () => {
    ...
    const verifier = createVisibleRecaptchaVerifier(recaptchaId);
  ...
  return (
    <div className={`rounded-sm border border-line bg-bg p-6 space-y-5 ${className}`}>
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-widest text-ink">SMS Multi-Factor Authentication</p>
          ...
          {smsMfaFactors.map((factor) => (
            <div key={factor.uid} className="flex flex-col gap-3 rounded-sm border border-line bg-surface p-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm font-black uppercase tracking-wide break-words text-ink">{factor.phoneNumber || 'SMS number on file'}</p>
        ...
        <>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="space-y-2">
              <label htmlFor={`${recaptchaId}-phone`} className="label-micro">Mobile Number For SMS Codes</label>
              <input
            ...
            <div className="space-y-2">
              <label htmlFor={`${recaptchaId}-device`} className="label-micro">Device Label</label>
              <input
                id={`${recaptchaId}-device`}
                type="text"
                className="input-industrial w-full"
          ...
          {mfaVerificationId ? (
            <div className="rounded-sm border border-line bg-surface p-4 space-y-4">
              <div>
                <p className="text-xs font-black uppercase tracking-widest text-ink">Confirm SMS Code</p>
                <p className="mt-2 text-sm leading-6 text-muted">
